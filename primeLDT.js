...
let expName = 'primeLDT';  // from the Builder filename that created this script
let expInfo = {
...
async function experimentInit() {
  ...
  // Initialize components for Routine "repeater"
  repeaterClock = new util.Clock();
  // Initialize components for Routine "whatsInvolved"
  whatsInvolvedClock = new util.Clock();
  ...
  key_resp_02 = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
...
function consentRoutineBegin(snapshot) {
  return async function () {
    ...
    consentComponents = [];
    consentComponents.push(text_1);
    consentComponents.push(key_resp_1);
...
function trials_3LoopBegin(trials_3LoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisTrial_3 of trials_3) {
      ...
      trials_3LoopScheduler.add(repeaterRoutineBegin(snapshot));
      trials_3LoopScheduler.add(repeaterRoutineEachFrame());
      trials_3LoopScheduler.add(repeaterRoutineEnd(snapshot));
      const consentloopLoopScheduler = new Scheduler(psychoJS);
      trials_3LoopScheduler.add(consentloopLoopBegin(consentloopLoopScheduler, snapshot));
      trials_3LoopScheduler.add(consentloopLoopScheduler);
      trials_3LoopScheduler.add(consentloopLoopEnd);
...
function consentloopLoopBegin(consentloopLoopScheduler, snapshot) {
  return async function() {
    ...
    consentloop = new TrialHandler({
      psychoJS: psychoJS,
      nReps: nRepsConsent, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: undefined,
    ...
    for (const thisConsentloop of consentloop) {
      ...
      consentloopLoopScheduler.add(whatsInvolvedRoutineBegin(snapshot));
      consentloopLoopScheduler.add(whatsInvolvedRoutineEachFrame());
      consentloopLoopScheduler.add(whatsInvolvedRoutineEnd(snapshot));
      consentloopLoopScheduler.add(confidentialityRoutineBegin(snapshot));
      consentloopLoopScheduler.add(confidentialityRoutineEachFrame());
      consentloopLoopScheduler.add(confidentialityRoutineEnd(snapshot));
      consentloopLoopScheduler.add(consentFormRoutineBegin(snapshot));
      consentloopLoopScheduler.add(consentFormRoutineEachFrame());
      consentloopLoopScheduler.add(consentFormRoutineEnd(snapshot));
...
function repeaterRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'repeater' ---
    t = 0;
    repeaterClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    // keep track of which components have finished
    ...
    for (const thisComponent of repeaterComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function repeaterRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'repeater' ---
    // get current time
    t = repeaterClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
...
function repeaterRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'repeater' ---
    for (const thisComponent of repeaterComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    }
    // the Routine "repeater" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
function whatsInvolvedRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'whatsInvolved' ---
    t = 0;
    whatsInvolvedClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    key_resp_02.keys = undefined;
    key_resp_02.rt = undefined;
    _key_resp_02_allKeys = [];
    // keep track of which components have finished
    whatsInvolvedComponents = [];
    whatsInvolvedComponents.push(text_2);
    whatsInvolvedComponents.push(key_resp_02);
    
    for (const thisComponent of whatsInvolvedComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function whatsInvolvedRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'whatsInvolved' ---
    // get current time
    t = whatsInvolvedClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    ...
    // *key_resp_02* updates
    if (t >= 0.0 && key_resp_02.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      key_resp_02.tStart = t;  // (not accounting for frame time here)
      key_resp_02.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { key_resp_02.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { key_resp_02.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { key_resp_02.clearEvents(); });
    }

    if (key_resp_02.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp_02.getKeys({keyList: ['space'], waitRelease: false});
      _key_resp_02_allKeys = _key_resp_02_allKeys.concat(theseKeys);
      if (_key_resp_02_allKeys.length > 0) {
        key_resp_02.keys = _key_resp_02_allKeys[_key_resp_02_allKeys.length - 1].name;  // just the last key pressed
        key_resp_02.rt = _key_resp_02_allKeys[_key_resp_02_allKeys.length - 1].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function whatsInvolvedRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'whatsInvolved' ---
    for (const thisComponent of whatsInvolvedComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    ...
    key_resp_02.stop();
    // the Routine "whatsInvolved" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
function confidentialityRoutineBegin(snapshot) {
  return async function () {
    ...
    confidentialityComponents.push(text_15);
    confidentialityComponents.push(key_resp_9);
...
function confidentialityRoutineEnd(snapshot) {
  return async function () {
...
function consentFormRoutineBegin(snapshot) {
  return async function () {
    ...
    consentFormComponents.push(text_18);
    consentFormComponents.push(key_resp_12);
...
function consentFormRoutineEachFrame() {
  return async function () {
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    // *prime* updates
    if (t >= 1.0 && prime.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 1.0 + current_jitter - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
...
function trialRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData("prime_word", prime_word);
